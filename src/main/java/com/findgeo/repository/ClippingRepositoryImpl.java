...
public class ClippingRepositoryImpl implements ClippingRepositoryCustom{
	...
	public List<MyClipDto> findByEmailQuerydsl(Long clipid) {
		...
		List<MyClipDto> result =  queryFactory.select(
					Projections.constructor(MyClipDto.class,
							clipping.clipid,
							clipping.areaname,
							clipping.congest,
							clipping.email,
				...
				.from(clipping)
				.leftJoin(trafmark).on(clipping.clipid.eq(trafmark.clipping.clipid))
				.leftJoin(fdmark).on(clipping.clipid.eq(fdmark.clipping.clipid))
				.leftJoin(retamark).on(clipping.clipid.eq(retamark.clipping.clipid))
				.leftJoin(svmark).on(clipping.clipid.eq(svmark.clipping.clipid))
				.leftJoin(armark).on(clipping.clipid.eq(armark.clipping.clipid))
				.where(clipping.clipid.eq(clipid))
				.fetch();
